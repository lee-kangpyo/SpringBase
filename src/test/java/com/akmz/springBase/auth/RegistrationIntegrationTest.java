...
public class RegistrationIntegrationTest {
    ...
    @BeforeEach
    void setUp() {
        String uniqueId = UUID.randomUUID().toString().substring(0, 8); // UUID의 앞 8자리만 사용
        validRequest = new UserRegistrationRequest();
        validRequest.setUserName("testuser_" + uniqueId);
        validRequest.setEmail("test_" + uniqueId + "@example.com");
    ...
    void testSuccessfulRegistration() throws Exception {
        ...
        assertThat(authMapper.findByUsername(validRequest.getUserName())).isNotNull();
        assertThat(authMapper.existsByEmail(validRequest.getEmail())).isTrue();
    }
    ...
    void testRegistrationWithInvalidEmail() throws Exception {
        ...
        mockMvc.perform(post("/api/auth/register")
                ...
                .andExpect(jsonPath("$.errors.email").value("올바른 이메일 형식을 입력해주세요."));
    }
    ...
    void testRegistrationWithWeakPassword() throws Exception {
        ...
        mockMvc.perform(post("/api/auth/register")
                        ...
                        .content(objectMapper.writeValueAsString(validRequest)))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.errors.password").value("비밀번호는 영문 대소문자와 숫자를 포함해야 합니다."));
    }
    ...
    void testRegistrationWithDuplicateEmail() throws Exception {
        ...
        duplicateEmailRequest.setEmail(validRequest.getEmail()); // 중복 이메일
    ...
    void testRegistrationWithDuplicateUsername() throws Exception {
        ...
        duplicateUsernameRequest.setUserName(validRequest.getUserName()); // 중복 사용자명
